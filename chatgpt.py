    ...
    try:
     ...
     # Catch OpenAI API errors
        ...
        print(colorama.Fore.YELLOW +
              f"ChatGPT: Rate limit reached - {e}" + colorama.Style.RESET_ALL)
        print("Please try again in 20s.")
        time.sleep(20)
        break  # Break out of the loop to avoid continuing with an invalid OpenAI API request
        ...
        print(colorama.Fore.YELLOW + "ChatGPT: I'm busy, only 3 requests per minute are allowed for free accounts,"
              " please ask your question again in 20 seconds" + colorama.Style.RESET_ALL)
        time.sleep(20)
        break  # Break out of the loop to avoid continuing with an invalid OpenAI API request
